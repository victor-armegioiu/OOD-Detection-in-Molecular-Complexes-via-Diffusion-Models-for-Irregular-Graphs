...
def main():
    ...
    try:
        ...
        if args.mode == 'single':
            ...
            # 3. Train the model
            best_metrics, early_stopped, optimizer, scheduler, scaler, epoch = train_model(model, train_data, eval_data, CONFIG)
            
            # 4. Save checkpoint
            save_checkpoint(model, CONFIG, save_path=CONFIG['checkpoint_path'].replace(".pt", "_final.pt"), optimizer=optimizer, scheduler=scheduler, scaler=scaler, epoch=epoch, best_metrics=best_metrics)
