...
def process_dataset(dataset, evaluator, device, dataset_name, output_path):
    ...
    for batch in dataloader:
        ...
        # Conditioning state coordinates - Scale and remove COM, add offset to the node IDs
        # ---------------------------------------------------------------------------------
        cond_coords_lig = batch.initial_ligand_coords / evaluator._model.scheme.coord_norm
        cond_coords_poc = batch.initial_pocket_coords / evaluator._model.scheme.coord_norm  
        cond_ligand_mask = batch.initial_ligand_coords_batch
        cond_pocket_mask = batch.initial_pocket_coords_batch

        # Remove COM from the cond coordinates
        combined_cond_coords = torch.cat([cond_coords_lig, cond_coords_poc], dim=0)
        combined_cond_mask = torch.cat([cond_ligand_mask, cond_pocket_mask])
        combined_cond_coords_centered = remove_mean_batch(combined_cond_coords, combined_cond_mask)

        # Split the centered coordinates into ligand and pocket
        cond_coords_lig = combined_cond_coords_centered[:cond_coords_lig.shape[0]]
        cond_coords_poc = combined_cond_coords_centered[cond_coords_lig.shape[0]:]

        # Add offset to the node IDs
        cond_node_ids_atoms = batch.cond_node_ids_atoms + cond_ligand_mask * 100000
        cond_node_ids_residues = batch.cond_node_ids_residues + cond_pocket_mask * 100000
        # ---------------------------------------------------------------------------------


        # Main state coordinates - Scale and remove COM, add offset to the node IDs
        # ---------------------------------------------------------------------------------
        ...
        # Remove COM from the main coordinates
        combined_coords = torch.cat([lig_coords, pocket_coords], dim=0)
        combined_mask = torch.cat([ligand_mask, pocket_mask])
        combined_coords_centered = remove_mean_batch(combined_coords, combined_mask)

        # Add offset to the node IDs
        node_ids_atoms = batch.node_ids_atoms + batch.lig_coords_batch * 100000
        node_ids_residues = batch.node_ids_residues + batch.prot_coords_batch * 100000
        # ---------------------------------------------------------------------------------
